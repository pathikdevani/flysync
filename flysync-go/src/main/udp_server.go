...
func (server *UDPserver) start()  {
	...
	for{
		...
		if(!isMyIp(add.IP.String())){
			...
			switch msg[0] {
			case "FLYSYNC_SERVER_FIND_REQUEST":
				add.Port = 8080
				Conn,_ := net.DialUDP("udp",serverAddress,add)
				Conn.Write([]byte("FLYSYNC_SERVER_FIND_RESPONSE"))
				Conn.Close()
			case "FLYSYNC_SERVER_FIND_RESPONSE":
	...
	for _,i := range interfaces{
		...
		for _,add := range adds{
			...
			if myIp.To4() != nil{
				...
				sendUdpPacket("FLYSYNC_SERVER_FIND_REQUEST",brodcastIp.String() +":" + port)
			}
		}
	}
}
